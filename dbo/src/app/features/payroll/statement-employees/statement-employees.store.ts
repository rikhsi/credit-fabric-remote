...
import { CardStatus, cardStatusIcons } from '../../../shared/models/card-status';
...
@Injectable()
export class StatementEmployeesStore {
  ...
  filterConfig = signal<FilterConfig[]>([
    ...
    },
    {
      ...
      type: 'select',
      placeholder: 'Статус',
      options: Object.keys(cardStatusIcons).map(item => ({
        label: cardStatusIcons[item].label,
        value: item,
        iconName: cardStatusIcons[item].icon,
        svgClass: cardStatusIcons[item].svgClass
      }))
    },
  ]);
  ...
  filters = signal<
    { type: CardType | '', searchText: string, status: CardStatus | '' }
  >(
    { type: '', searchText: '', status: '' },
    {
      equal: (a, b) =>
        a.type === b.type &&
        a.status === b.status &&
  ...
  readonly employeeListAll = toSignal(
    combineLatest([
      ...
      tap(res => {
        if (res) this.total.set(res.totalElements);
  ...
  readonly employeeListFiltered = computed(() => {
    const list = this.employeeListAll();
    const term = this.filters().searchText.trim().toLowerCase();

    if (!list) return [] as SalaryCardRes[];
    if (!term) return list.content;

    return list.content.filter((item) =>
      item.ownerName.toLowerCase().includes(term) ||
      item.pan.toLowerCase().includes(term)
    );
  });

  setFilter(filter: { type: CardType | '', searchText: string, status: CardStatus | '' }) {
    this.search.set(filter.searchText);
    this.filters.update(value => ({ ...value, ...filter }))
  }
}
