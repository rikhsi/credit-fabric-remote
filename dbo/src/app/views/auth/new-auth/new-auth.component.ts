...
export class NewAuthComponent implements OnInit, OnDestroy {
  serialNumber = signal('')
  ...
  checkUser() {
    let {username, password, espKey, code} = this.loginForm.getRawValue();
    this.utilsService.spinnerState$$.next(true);
    this.authService.getUserBusiness(username, password)
      .pipe(takeUntil(this.unsub$))
      .subscribe({
        next: (res: any) => {
          if(res) {
