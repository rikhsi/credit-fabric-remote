...
import {
  CONVERSION_TABS, ConversionKey,
} from 'src/app/views/main/features/new-main/constants/new-main.const';
...
} from "../../../mass-payments/components/created-payments/components/payments/components/payment-filter/payment-filter.component";
import {
  PaymentListItemComponent
} from "../../../mass-payments/components/created-payments/components/payments/components/payment-list-item/payment-list-item.component";
import {BalanceComponent} from "../../../main/components/balance/balance.component";
import {
  GetPaymentImportFileDataReq,
  GetPaymentImportFileDataResContent
} from "../../../mass-payments/models/mass-payments.model";
...
import {ConversionFilterComponent} from "./filter/conversion-filter.component";
...
export  class PaymentsComponent implements OnInit,OnDestroy {
  ...
  public readonly activeTabs = signal<ConversionKey>('all');
  ...
  private getCurrentTabsDatas():Observable<any> {
    ...
    }else if(this.activeTabs() == 'errors') {
      // return this.getPaymentImportFileData(this.paymentId())
    }
    return EMPTY
  }
  ...
  private initFilters() {
    ...
    const route$ = combineLatest([
      ...
      tap(([id, queryParams]) => {
        ...
        this.activeTabs.set(
          (queryParams.get('type') as ConversionKey) || 'all'
        );
      }),
      shareReplay(1)
    );
  ...
  setPaymentActiveTab(k: ConversionKey) {
  ...
  //   this.pageSize.set(this.DEFAULT_PAGINATION_SIZE);
  ...
  redirectToCreateForm(type: string) {
   return this.route.navigate(['/conversion/create-payment'], {
      queryParams: {
        type: type
      }
    });
  ...
  protected readonly CONVERSION_TABS = CONVERSION_TABS;
}
