...
@Component({
  ...
  templateUrl: './deposit-payment-item.component.html',
  styles: `
    .leading-trim {
      leading-trim: both;
      text-edge: cap;
...
export class DepositPaymentItemComponent {
@Input() transaction!: any;
 ...
 formatDate(createdAt: string): string {
  ...
  subscribe(event: Event) {
    ...
    console.log('Transaction ID for signing:', this.transaction.id);
  }
  ...
  onSubAction(action: { id: string; title: string }) {
    this._matDialog.open(EspSignConfirmComponent, {
      ...
      .subscribe({
        next: res => {
          this.console.log('Dialog closed with result:', res)
          if (res === 'update') {
