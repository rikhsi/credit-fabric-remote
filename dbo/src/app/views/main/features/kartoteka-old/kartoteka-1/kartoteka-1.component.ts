...
export class Kartoteka1Component implements OnInit {
  ...
  activeCardsLength = computed(() => {
    if (this.activeTab() === 'ACTIVE') {
      localStorage.setItem("acTiveKartoteka1Length", JSON.stringify(this.kartoteka2Store.content().length ?? 0))
  ...
  protected loadList(filter?: FilterRequest) {
    ...
    this.kartoteka2Store.loadKartoteka2List({ paging: { page: this.kartoteka2Store.pageNumber(), size: this.kartoteka2Store.pageSize() }, cardFileType: 1, filter: finalFilter })
  }
