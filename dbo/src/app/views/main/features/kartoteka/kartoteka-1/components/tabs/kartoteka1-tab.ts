...
export type TabValue = 'ALL' | 'ACTIVE';
...
@Component({
  selector: 'kartoteka1-tab',
  imports: [NgClass, NgIf, TranslateModule, MatTooltip],
  templateUrl: './kartoteka1-tab.html',
  changeDetection: ChangeDetectionStrategy.OnPush
})


export class Kartoteka1TabComponent  {
  ...
  private isValidTab(tab: string): tab is TabValue {
    return ['ALL', 'ACTIVE'].includes(tab);
  }
  ...
  protected downloadExcel() {
    this.kartotekaService
      .getFiles({
        fileType: "EXCEL",
        cardFileType: this.baseKartotekaStore.payload()?.cardFileType ?? 1,
      ...
      .subscribe((transaction) => {
        if (transaction?.file) {
          ...
          link.download = `kartoteka1-file.xlsx`;
          link.click();
