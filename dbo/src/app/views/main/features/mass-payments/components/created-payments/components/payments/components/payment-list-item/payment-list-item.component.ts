...
// payment-list-item.component.ts
...
import { CustomTooltipComponent } from 'src/app/shared/components/custom-tooltip/custom-tooltip.component';
...
import { CreatedPaymentsKey } from 'src/app/views/main/features/new-main/constants/new-main.const';
...
export class PaymentListItemComponent implements OnInit , OnChanges {
  ...
  tabType = input.required<CreatedPaymentsKey>()
...
showDetails(event: Event): void {
  ...
  private handleTransactionMode(transaction) {
      if(this.tabType() == 'all' || this.tabType() == 'creation-errors') {
